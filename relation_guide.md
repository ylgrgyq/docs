...
决策的关键在于是否需要为这个关系附加一些属性。如果不需要，使用 AVRelation 或 Arrays 是最简单的。通常情况下，使用 Arrays 会有更好的性能和更少的查询。如果多对多关系中任何一方对象数量可能达到或超过100，像一对多关系中描述的原因一样，使用 AVRelation 或关联表是更好的选择。
...
假设我们一个读书应用拥有`Book`和`Author`对象。`Book`对象包含一个`Author`对象的数组（使用列名`authors`）。使用 Arrays 实现非常适合这样的场景，因为一本书几乎不可能达到或超过100个作者。这是我们在`Book`对象中使用数组的原因。毕竟，一个作者可能写100本以上的书。
...
* **限制部分用户数据的权限** 在这个场景中，你可以将此对象拆分成两部分，一部分包含所有其他用户可见的数据，另一部分包含所有仅自己可见的数据（通过ACL控制）。
