...
新的 Cocoapods 不支持静态链接的 framework，客户端工具也不能发布静态链接的 framework，而动态链接的 framework 只能在 iOS8 上使用，所以我们目前已经暂停了 Cocoapods 安装的支持，所有新版 iOS sdk 都需要到我们官网下载。
...
### 短信的免费 100 条额度是帐户内还是每个应用？

每个 Leancloud 帐户拥有 100 条免费的短信测试额度，不是每个应用。使用完毕就需要付费。可以通过[充值](https://leancloud.cn/bill.html#/bill/charge)菜单来购买短信。

### 短信的到达率如何？

我们通过接入多个短信提供商来提升短信的到达率，从我们实际使用来看，整体的到达率还是令人满意的。但是请注意，短信是无法保证 100% 送达，你的程序的重要功能除了短信之外，应该有其他备份手段来提供给用户。
...
* 针对同一个手机号码，一分钟只能发送一个验证码短信
* 每天只能发送 10 个验证短信给同一个手机号码。
...
有的，请在[开发者帐户](https://leancloud.cn/settings.html#/setting/info)里填写手机号码，我们将提前通过邮件和短信的方式发送告警信息给您。
