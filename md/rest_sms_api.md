...
## 图片验证码及其重要性
### 短信轰炸
网络上并不全是好人的世界，在每个角落都可能充斥各种不法分子，短信轰炸就是一例。

网络上有这样一种软件存在，它可以自动收集一些不需要认证（如图片验证码认证）就能发送短信验证码的网站，我们称它为「轰炸软件」。一些不法之徒就利用这样的轰炸软件，随意输入一个或一组手机号码，轰炸软件首先找到一大堆可以发送短信验证码的网站逐个访问，然后在每一个网站上把手机号都自动填上，这些网站就会同时往这个手机号码发短信了。轰炸软件找的网站越多，发的短信也就越多。

轰炸软件的恶意之处还在于，它对各个网站的短信验证码请求不仅仅是一次，它可以设置时间，比如每隔几分钟请求一次，24小时不间断请求，这样造成的后果就是：

- 对于网站来说会不停的发出短信验证码；
- 对手机号码来说会在短时间内频繁的接收到各个无关网站的短信验证码。

如何防范这种攻击？

### 图片验证码
毫无疑问，图片验证码是防范短信轰炸最有力的手段。什么是图片验证码？可以参考下面这张图：

![image](images/captcha.png)

我们在一些网站注册的时候，经常需要填写以上图片的信息。这种图片验证方式是我们最常见的形式，如果网站在用户进行「免费获取验证码」 操作前，要求用户输入图片验证码，确认用户是真实有效后，服务器端再请求 LeanCloud 云端发送动态短信到用户手机上，就可以有效防范恶意攻击者。

正确的加入图片验证码的方式是在短信验证码发送前，先让用户填写图片验证码，再发送短信验证码。看一个正确的例子（下图）：

![image](images/captcha.png)

和一个错误的例子（下图）：

![image](images/captcha_wrong.png)
...
除了采用这种图片输入的方式验证，现在很多网站的也采用了很多新颖的验证方式，如：有很多网站采用行为验证方式，采用拖拽验证方式，如淘宝注册，pptv注册等。目前，也有一些公司提供图片验证码类解决方案，大家都可以尝试一下。
