...
### 国际短信
上面发送短信验证码和语音验证码，默认都是对国内号码。LeanCloud 现在已经开通了国际短信验证码服务（语音验证码海外还不可用）。要发送国际短信，只需要在发送 `https://api.leancloud.cn/1.1/requestSmsCode` 请求的时候，额外加上**countryCode**这一参数项即可。`countryCode`的取值范围请参考[这里](https://countrycode.org/)，例如 `US` 表示美国，`CN` 代表中国。
