...
### 数据
对数据进行管理，包括 Class 的创建，删除，数据的增删改查，支持数据导入，[Class 绑定](app_data_share.html)，[应用内搜索](app_search_guide.html)。
...
这里的「定义函数」和「Git 部署」是实现云代码部署相关的两种方式。两种方式是相斥的，即通过在页面里 「定义函数」会覆盖掉通过「Git 部署」的代码，反之亦然。
#### 定义函数
...
#### 定时任务
对通过前面两种方式定义的云代码，作定时处理。 [相关文档](cloud_code_guide.html#定时任务)

#### 日志
可以查看云代码的输入日志，区分生产和测试环境。

#### 统计
云代码相关统计项，可以查看函数，页面相关的调用情况，也可以统计云代码主机相关运行参数。
...
### 离线数据分析
通过 SQL 对数据进行分析。需要先开启「离线数据分析」 选项。分析完成后可以将分析结果保存为 Class 或导出。
...
## 分析
* 支持各种常见统计指标
* 支持按照系统类型，系统版本，渠道版本，自定义用户群，时间段进行统计，支持按时间段对比分析（仅限对有对比意义的统计项）
* 支持实时统计一些关键数据
...
### 应用内搜索
类似全文搜索，[相关文档](app_search_guide.html)，在这里需要设置一些基本信息。
...
应用墙发布设置：审合通过后，会发布到 [我们的用户页面](/customers.html)
