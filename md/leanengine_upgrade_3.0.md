...
关于 `package.json` 更多的信息，可以参考 [这里](https://docs.npmjs.com/files/package.json)。
...
如果是按照 [创建 package.json 文件](#创建 package.json 文件) 章节描述的操作，`package.json` 文件应该已经有 `leanengine` 的依赖了。如果没有请在 `$PROJECT_DIR` 目录下执行：
...
* 如果想了解具体有哪些环境变量，可以参考 [这里](https://leancloud.cn/docs/leanengine_guide-node.html#环境变量)。

### 提供 Web 服务

第 14 ~ 19 行，引入 `app` 模块，这个模块 [下面](#app 模块) 描述，然后使用环境变量提供的端口启动应用。
...
因为 3.0 项目不收沙箱限制，不需要强制将模板目录保存在 `$PROJECT_DIR/cloud/views` 目录下，取而代之使用更加通用的习惯，保存在 `$PROJECT_DIR/views`，所以改成：
...
>44 });
 45
>46 // error handlers
>47 app.use(function(err, req, res, next) { // jshint ignore:line
...
* 未捕获异常处理器：第 17 ~ 31 行，关于 domain 的用法详见 [这里](https://nodejs.org/api/domain.html#domain_domain)。
...
回忆下我们在 [`server.js`](#创建 server.js) 部分引用了 `app` 并调用了 `app.listen` 方法，所以 `$PROJECT_DIR/app.js` 可能会是这个样子（第 53 行）：
...
**注意**：3.0 项目中**不能**存在 `$PROJECT_DIR/cloud/main.js` 文件，否则会被当做 2.0 的项目加载并运行。2.0 和 3.0 的项目约束可以分别参考 [这里](https://leancloud.cn/docs/leanengine_guide-cloudcode.html#项目约束) 和 [这里](https://leancloud.cn/docs/leanengine_guide-node.html#项目约束)。
...
* 第 1 行不需要在引入 `app.js`，取而代之是引入 `leanengine` 模块，然后就可以使用 `AV.Cloud.define` 方法来定义 Cloud 函数了。
...
**注意**：`cloud` 模块必须引入，而且应该在 Express 的中间件链尽量靠前的位置。因为该中间件会提供一个健康监测的 URL，LeanEnging 监控服务会根据该 URL 的响应判断应用是否启动成功。放在后面很容易被其他的 router 替代，导致响应不是预期而认为启动失败。关于健康监测的详细信息请参考 [这里](https://leancloud.cn/docs/leanengine_guide-node.html#健康监测)。
...
即不需要在引入 `avos-express-cookie-session` 中间件，不需要使用 `cookieParser`，直接使用 `AV.Cloud.CookieSession` 的方式来配置，具体信息参见 [这里](https://leancloud.cn/docs/leanengine_guide-node.html#处理用户登录和登出)
