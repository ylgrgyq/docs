...
几乎每一位使用 LeanCloud 的用户都会问一下，如何能够保证自己应用的安全？如果你在读这份文档，相信你也是其中之一。对安全的关注说明你也和我们一样，都是对产品负责的、对用户负责的、对自己负责的、一个「认真」的人，这也是「[LeanCloud 的价值观](http://open.leancloud.cn)」。安全界有个说法「绝对的安全是不存在的」，关键的点是要梳理清楚「安全边界」，安全边界就是你对于业务安全范围的一个清晰的界定。
