...
LeanCloud 总体上通过 `SSL 安全连接（HTTPS）`、`Web 安全域名`、`ACL 权限控制`，三重级别来总体控制每个应用的安全。数据层面每天备份一次数据，防止误操作等情况删除重要数据。当然，我们还有其他很多细节的安全设置，还有很多服务器端验证和检测，比如实时通信中可以支持自定义的服务器认证、短信验证的各种安全设置、SDK 中的各种安全细节等，这些可以在你的使用过程中不断学习和了解。那么接下来简单说下，应用安全是如何保证的。
...
通过 `服务开关` 可以开启或者关闭当前 App 不使用的其他服务，从根本上防止 AppId 和 AppKey 泄露后，可能的服务资源盗取问题。
