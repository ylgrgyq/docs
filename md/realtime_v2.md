...
开发者可以通过 LeanCloud 控制台的 **消息** > **实时消息** > **帮助** 界面查询某个 Client ID 的在线状态和离线消息数。
...
`4111`|`SESSION_CONFLICT`|启用单设备登录后，帐号在别的设备上登录，当前设备被强制下线。
...
请先参考 [聊天离线时可以推送吗](#聊天离线时可以推送吗)。在控制台的设置页面，填写「您有新的未读消息」后，当对方不在线的时候，便会触发一个 APNs 的推送。首先，请确保控制台能向 iOS 推送消息，也即如下图所示的推送能顺利到达 iOS 系统，请参考 [iOS 推送开发文档](ios_push_guide.html)。
