...
系统对话给用户发送的消息记录查询请参考： [获取系统对话中某个特定用户与系统的消息记录](https://leancloud.cn/docs/realtime_rest_api.html#获取系统对话中某个特定用户与系统的消息记录)

应用首次有用户发送消息给某系统对话时，我们会创建`_SysMessage`表，并且将所有发送到系统对话的消息都存储在该表中。用户发送给系统对话的消息记录就可以在这个表中进行查询。除此之外，开发者还可以自定义 Web Hook 来实时接收用户发给系统对话的消息。
