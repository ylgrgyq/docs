...
请注意，**开启混合推送** 选项可以根据您的需要随时安全的切换。比如因为某些原因不想使用混合推送了可以将该选项关闭，关闭后下一次 Android 推送会跟普通推送一样自动走 LeanCloud 自有通道送达客户端，除了会再次受到上面提到的自有通道在部分 ROM 上的限制之外不会有别的影响。等该选项再次切换为开启后，Android 推送又会再次走第三方推送渠道。
