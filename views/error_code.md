...
* 含义 - 超过应用的流控限制，每个应用同一时刻的并发请求上限默认为 30（即同一时刻最多可以同时处理 30 个数据请求）；可以在 API 统计的性能统计总览中看到你的应用的请求统计，例如平均并发、平均响应时间等。如有必要可以主动联系我们提升。
...
* 含义 - 超过云引擎 hook 调用流控限制，通常绝大多数应用不会触发此限制。如有遇到，请联系我们处理。可以在 API 统计的性能统计慢查询中可以看到应用的云引擎 Hook 请求状况。
