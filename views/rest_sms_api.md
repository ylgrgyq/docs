...
我们为每个 LeanCloud 账户提供 100 条国内短信的免费额度进行测试，超过的部分将实时从账户余额中扣除，所以请务必保证账户余额充足。具体的价格请参看 [官网价格](/pricing.html)。
...
### 测试手机号与固定验证码
对于部分使用「手机号 + 验证码」登录的应用来说，在其应用上架前提交苹果审核的过程中，可能面临苹果测试人员无有效手机号码无法登录、开发者又无法提供固定手机号和验证码的尴尬境况。

有些用户在开发过程中，也会面临短时间内、多次登录／注销的测试操作，这时候验证码的时间间隔与总次数限制，可能会造成种种不变。

为满足这些需求，我们提供了设置测试手机号和固定验证码的功能。每个应用都可以增加一个测试手机号，LeanCloud 平台会为它生成一个固定的验证码，每次使用手机号+固定验证码进行验证都会得到成功的结果。这样可以便捷的满足特定用户登录测试以及应用审核等场景需求。

在 [应用控制台](/applist.html#/apps) > **消息** > **短信** > **设置** > **其他** 里可以设置测试手机号。
...
短信有 **验证**（包括语音验证）和**通知**（如通过短信模板创建）。由于运营商和渠道的限制，短信发送要遵循以下原则：
...
从 [控制台 > **消息** > **短信** > **发送记录**](/messaging.html?appid={{appid}}#/message/sms/create) 可以清楚地看到每条消息和它的发送状态。
...
3. [模板语法正确](sms_guide-objc.html#模板变量)：我们仅支持 handlebars 语法，变量是以两个大括号括起来，类似 {{ docs.mustache("var") }}。
4. 默认短信签名是应用名，你也可以在模板里设置短信签名，签名的要求请参考 [短信签名说明](sms_guide-objc.html#短信签名说)。
