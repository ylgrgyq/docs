...
云引擎运行的时候，会首先加载 `wsgi.py` 这个模块，并且将此模块的全局变量 `application` 做为 WSGI 函数进行调用。因此请保证你的 `wsgi.py` 文件中包含一个 `application` 的全局变量／函数／类，并且符合 `WSGI` 规范。

关于 `WSGI 函数`，更多详情请参考[此篇文章](http://www.liaoxuefeng.com/wiki/0014316089557264a6b348958f449949df42a6d3a2e542c000/001432012393132788f71e0edad4676a3f76ac7776f3a16000) 或者 [PEP333](https://www.python.org/dev/peps/pep-0333/)。
...
详细格式参考 [pip 文档相关章节](https://pip.pypa.io/en/stable/user_guide/#requirements-files)。

应用部署到云引擎之后，会自动按照 `requirements.txt` 中的内容进行依赖安装。在本地运行和调试项目的时候，可以在项目目录下使用如下命令进行依赖安装：

```sh
pip install -r requirements.txt
```

另外在你应用部署的时候，建议将依赖的包的版本都使用 `foo==1.0.0` 这种形式来指定明确的版本号（或者指定一个版本范围），防止因为依赖的模块升级并且不再兼容老的 API，再次部署的时候导致应用运行失败。

### 指定 Python 版本

另外你还可以选择运行代码的 Python 版本，目前支持 3.5 和 2.7 两个版本。
...
我们提供了 Flask 和 Django 的两个框架的实例项目，可供参考，或者直接作为你应用项目的初始化模版：
...
在云引擎中你可以使用 LeanCloud 提供的数据存储来作为应用的后端数据库，以及使用其他 LeanCloud 提供的功能。使用 LeanCloud Python SDK 可以更加方便地使用这些功能。
...
因为 `wsgi.py` 是项目最先被执行的文件，推荐你在此文件进行 LeanCloud Python SDK 的初始化工作：
...
接下来就可以在项目其他部分使用 LeanCloud Python SDK 提供的功能了。更多关于 LeanCloud Python SDK 的文档请参考[这里](leanstorage_guide-python.html)。

## 本地运行和调试

在本地运行 LeanEngine Python 应用，首先需要这几个依赖：

- Python，目前云引擎线上支持 2.7 / 3.5 两个 Python 版本，最好确保你本地安装的 Python 版本与线上使用的相同
- pip，用来安装第三方依赖
- virtualenv，可选，建议你使用 virtualenv 或者类似的工具来创建一个独立的 Python 环境，以免项目使用到的依赖与系统／其他项目的版本产生冲突

请确保你的机器上已经安装好以上依赖，然后在项目目录下执行如下命令，来安装项目用到的第三方依赖：

```sh
pip install -r requirements.txt
```

然后便可以在项目目录，用我们的命令行工具来启动本地调试了：
...
# 使用 `leancloud.HttpsRedirectMiddleware` 这个 WSGI 中间件包装一下原始的提供给 LeanEngin 的 WSGI 函数
