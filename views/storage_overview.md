...
几乎所有的应用都会用户的概念，用户的功能比较通用，比如手机号注册用户，邮箱注册等，加上手机验证码登录，手机号一键登录等功能都是常见的功能接口，因此我们也封装了 AVUser 对象帮助开发者快速实现用户相关的功能。目前 AVUser 支持如下功能
...
* 邮箱+密码
* 手机号+密码
* 手机号+手机验证码（一键登录，如果之前用户有注册过就直接登录，如果没有就创建新的用户之后直接登录）
* 社交账号 OAuth 登录（QQ，微博，Facebook，Google 等）

还支持邮箱密码重置，手机密码重置等辅助的功能，详细需要参考 SDK 使用文档。
