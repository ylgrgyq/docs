...
- Visual Studio 2015 以上的版本
  微软针对 Unity 有一个工具包可以让 Unity 和 Visual Studio 联调，详细请看[使用 Visual Studio Tools for Unity](https://msdn.microsoft.com/zh-cn/library/dn940020.aspx)，因此建议用户在 Windows 上开发 Unity 的体验会更好，当然在 Mac OS 以及其他操作系统上开发也没有任何问题。
...
在继续阅读本文档之前，请先阅读[《实时通信开发指南》](https://leancloud.cn/docs/realtime_guide-objc.html)，了解一下实时通信的基本概念和模型。
...
目前 SDK 内置的 WebSocket 库是一个来自于开源社区的版本 [sta/websocket-sharp](https://github.com/sta/websocket-sharp)，它有个致命的缺陷就是：不支持 iOS 9.0 以上的版本，因此我们推荐开发者可以考虑购买一个付费的版本：[WebSocket for desktop, web and mobile](https://www.assetstore.unity3d.com/cn/#!/content/27658)，如果开发者有自己实现 WebSocket 客户端的能力也可以自己实现，并且 SDK 是支持在初始化的时候指定 WebSocket 客户端的。详细的可以阅读自定义配置中的[自定义 WebSocket 客户端](#自定义_SDK)
...
### 客户端 - AVIMClient
LeanCloud 实时消息中的客户端的指的是游戏中客户端于云端之间的单个长连接，而不局限于一个用户（玩家），例如 QQ 里面有一种机制就是一个 QQ 号支持同时在一台手机设备和一台电脑设备上登录，在 LeanCloud 实时消息中就把这两个登录的设备视为两个客户端，但是他们共享了一个客户端 ID - ClientId，只要是同一个 ClientId，不管多少个设备，LeanCloud 云端都可以支持。

#### 客户端的互斥
当然，游戏中更多的情况是，玩家在 iOS 上登录之后，他在其他设备上较早前的登录就会被踢下线，这是游戏的客户端独占性，因此游戏开发者可以完全等价的理解 AVIMClient 就是一个玩家，他们之间是一对一的关系。LeanCloud 定义这种行为叫做单点登录，如何实现单点登录请点击：[单点登录](#单点登录)
...
### 帮派和工会案例 - 桃园三结义
帮派（公会）是一个持久存在的某一些玩家共有的一个频道，它具备以下几个特点：

- 持久化存储
- 玩家加入或者离开时的通知送达
- 管理员可以加人和删人

经过分析，我们给出一个符合编程语义的定义，帮派或者公会它本质上是一个对话，这个对话具备如下几个属性特征：

- 非暂态对话
- 成员的变动通知
- 某一个特殊权限的客户端可以邀请或者提出其他客户端

假设我们开发一款三国题材的游戏，每一个玩家是三国时期的一个名将，游戏的第一个剧情就是桃园结义，因此我们假设有三个玩家分别叫 刘备（ID 1001）、关羽（ID 1002）、张飞（ID 1003），这三家玩家进入了游戏，他们偶然相遇，刘备一时兴起说道「我们结拜吧」，于是他们新建了一个帮派叫做「桃园」，下面以刘备作为玩家视角开始构建 AVIMClient 以及打开与 LeanCloud 云端的长链接并且创建「桃园」聊天频道的代码。
...
此时，三个人之间是没有任何联系的，他们之间暂时无法通讯，因此我们需要创建一个对话，让这三个人成为这个对话的成员然后开始三个人之间的群聊，这个群聊的名字就是我们之前提到的「桃园」：
...
#### 成员变更通知
对于关羽来说，刘备的新建了一个聊天频道，他此时也在线，因此只要他订阅了通知事件，刘备邀请他加入「桃园」聊天的这个操作，他就可以收到，如下是关羽登录的代码，请注意其中与刘备不同的地方：
...
注意，此时张飞正在睡觉，尽管他也会受邀加入对话，但是成员变更操作只针对在线人员进行通知，类似于 QQ 你多年没有上线，有一天你上线了，你发现你被某个群踢了，其实这个通知并不是在踢你的时候产生的，而是 QQ 根据你之前的群组记录对比你现在的群组列表，做了合并操作，发现你被踢了，就生成一个通知给你。
...
#### 接收消息
关羽此时已经登录了，并且已经知晓自己已经加入了「桃园」这个对话，因此他需要继续订阅接收消息的事件通知：
...
public void GuanYULogIn()
{
    ...
    }).ContinueWith(s =>
    {
        ...
        // 查询我所在的对话列表，默认返回的是最近活跃的 20 个，这个 20 是可选的，最大支持 1000
...
同样的是，刘备也需要监听 `AVIMClient.OnMessageReceived` 这个事件通知，就能收到关羽发送的消息了。

#### 离线消息
假设刚才所有的操作发生的时候，张飞依然在呼呼大睡，他完全不知道大哥创建了一个对话，并且大哥和二哥已经在对话里面聊了起来，此时他上线，他将会收到许多离线消息，这个时候张飞登录的时候，需要再监听一下离线消息的事件通知，这样他就不会丢失消息：
...
#### 邀请其他成员加入
刘备关羽张飞后来遇上了五虎上将的另一位赵云，刘备将赵云加入「桃园」中 ，首先需要获取「桃园」这个实例，第一种是上述关羽通过 `AVIMClient.GetQuery` 的方法来获取查询，然后通过名称比对来找出「桃园」的方法， 这个方法我们推荐在展现列表页的时候使用，如果一个对话很活跃，我们推荐开发者在客户端缓存对话的 ID(`AVIMConversation.ConversationId`)，然后通过 `AVIMConversation.CreateWithoutData` 来构建一个对话，这样效率更好，用法更友好： 
...
#### 剔除其他成员
等到后来在新野的时候，徐庶前来相投，刘备也将徐庶加入了「桃园」，但是无奈的是，曹操俘虏了徐庶的母亲，徐庶只能含泪告别刘备，在徐庶离开新野北上之后，刘备无奈地将徐庶从「桃园」中删除，他需要的代码如下:
...
### 私聊案例 - 曹操三问谋士
相对于刘备来说，曹操不太喜欢群聊，因此他总是单独约见某一个手下的谋士进行私聊，这里我们拿曹操与程昱，荀彧以及郭嘉的私聊来做案例。

前面我们说过，在 LeanCloud 中并没有限制对话的类型，群聊和私聊的本质区别就是成员的数量。

那么假设曹操的 Client ID 是 2001，程昱是 2002，荀彧是 2003，郭嘉是 2004。
...
增加 `category` 的标签，是为了日后在构建查询的时候，方便的传入这个条件，这样曹操就可以迅速的找到自己所有的私聊，例如下面就是曹操查询自己私聊的对话代码：
...
其他关于成员操作的通知，离线消息与刘备的「桃园」这个群聊没任何区别。

#### 私聊和群聊的隔离
假设曹操需要一个群聊来发布一些政令，他可以沿用前文关于创建群聊的代码，多数情况下，我们推荐用户创建一个新的对话，而不建议直接向原来的私聊添加成员，因为这会让聊天记录的获取显得比较尴尬，假设向私聊里面加入一个新成员，那么这个新成员是无权获取他加入之前的聊天记录的，因此我们十分建议用户将私聊和群聊从创建开始就区分开了，这样会在日后的开发中节省很多理解成本。

### 战场和副本案例 - 赤壁之战
时间来到赤壁之战，孙刘联盟，蔡中蔡和两兄弟假意投降，江干也被曹操派来做间谍，因此周瑜需要一个特殊的对话来实现临时的聊天需求：暂态对话。通俗地说就是聊天室，在这个聊天室里面有诸葛亮，有黄盖，有江干，有蔡中蔡和，周瑜是这个对话的创建人，因此他需要如下代码来实现这个需求：
...
这样创建出来的对话就是暂态对话，暂态对话和普通对话有如下几个不一样的设定：
...
#### 获取暂态对话
在上文介绍查询私聊的地方我们构建了一个对话的查询，我们可以继续使用该查询，只要修改查询条件就可以找出当前 client 所参与的所有暂态对话：
...
## 单点登录
游戏对应着多个客户端，例如 Android 客户端和 iOS 客户端，每一个客户端是互斥的，换言之，较早前登录的客户端会被新登录的设备踢下线，这个在聊天系统里面也是常见的，因此我们提供了实现这个需求的机制，具体代码如下：
...
- tag：标记这次登录的标签
- deviceId：当前登录的设备绝对不变的 Id，这个可以由开发者自行维护，也可以通过 API 去获取，云端不做校验

为了让开发者更好的理解参数，我们拿 QQ 做案例来类比，假设当前开发者在 Windows 上登录 QQ 桌面版客户端，那么在 SDK 中可以用如下代码模拟 QQ 的登录：
...
而当同一个 Client Id 使用相同的 tag 登录的时候，较早前登录的就会被云端强制踢下线，例如
...
#### 
Unity 的 聊天 SDK 中将 WebSocket 客户端的基本行为封装成了一个接口：
...
假设您自己实现了上述的接口，可以在初始化的时候指定给 SDK，这样 SDK 就会调用指定的 IWebSocketClient 用来访问聊天服务端：
...
### 自定义消息类型
之前所有的代码和实例都只演示了文本消息类型的发送和接收，在游戏中文本消息固然是主要的一种类型，但是更为普遍的是表情消息，一般情况下表情消息的实现是如下步骤：

1. 客户端将表情图片事先按照编号（类似于 #1，#2）存储在客户端的资源包里
2. 消息发送的时候，从本地资源包内加载表情列表，然后用户选择一个表情，点击发送
3. 发送的时候，并没有实际发送这个表情的物理文件，而是将编号发送给对方接收端
4. 接收端收到编号之后，在加载消息详情的时候，根据编号读取物理文件，显示出来


这个是传统网游在实现表情消息的逻辑，实际上可以看出：表情消息在代码层面本质上依然是个文本消息，接下来我们就来自定义一个表情消息。
...
然后用户可以跑一下之前刘备给关羽发消息的代码，然后在控制台可以看见如下的日志内容：
...
以 `websocket=>` 向右箭头的表示的是后面的这一行日志的内容是由客户端发往 LeanCloud 云端的，而以 `websocket<=` 向左箭头表示的是云端发往客户端的内容，从上面的日志可以看出，一条文本消息的主要内容就是 msg 这个字段里面的内容

```json
{"msg":{"_lctype":-1,"_lctext":"兄弟们，睡什么睡，起来嗨！"}}
```
一条文本消息的组成是两个字段：类型和文本内容
在 LeanCloud 实时通讯私有协议里面，带有下划线 `_lc` 是受保护字段，SDK 都会主动识别这个字段的含义。

因此不难想到，我们要实现一个表情消息，开发者可以自定义这个 `_lctype` 的值，比如设置成 1，
注意： `_lctype` 值负数都不建议开发者使用，开发者建议从 1 开始的正数。
...
`AVIMMessageClassName` 属性标记的就是最后 `_lctype` 的值，我们设定了是 1。

## 聊天记录
首先我们得十分谨慎的对待聊天记录，我们需要明确一个重要的设定：Unity 目前没有针对聊天纪录作本地的缓存，根据一般的游戏场景需求，每一次登录一般情况下，除了一些极少数的游戏提供了可以搜索私聊的聊天记录之外，很少有需求是玩家大量搜索聊天记录或者展现聊天记录，加上 Unity 操作 sqlite 不是很方便集成在 SDK 这一层，因此 LeanCloud Unity 实时通讯 SDK 是没有实现缓存功能的。

但是我们也提供了实时获取聊天记录的功能，这个功能因为很消耗服务端的性能，因此我们建议开发者可以自行实现本地缓存的功能（假设有需求的情况下），另外我们需要声明：我们暂时不限制用户应用中聊天记录的保存时间和条数。未来如有变化我们会提前通知用户，你也随时可以通过 REST API 将聊天记录同步到自己的服务器上。
