...
本接口主要针对使用 [AV.User](rest_api.html#用户-1) 的应用，可以方便快捷地实现登录认证。登录认证默认关闭，可以进入 [控制台 > 设置 > 应用选项](/app.html?appid={{appid}}#/permission)，在「聊天、推送」模块，勾选「聊天服务，启用登录认证」进行开启。
