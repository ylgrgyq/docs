...
我们对于每个应用都默认赠送一个「基本资源（0.5CPU/256M）」的体验实例，可以免费使用，供开发者学习和测试云引擎。

体验实例只存在于生产环境，在进行部署等管理操作时会暂停服务；同时体验实例会执行 [休眠策略](#休眠策略)，没有请求时会休眠，有请求时启动（首次启动可能需要几秒的时间），每天最多运行 18 个小时。

## 标准实例

对于商业项目和正式上线的产品，开发者可根据需要付费购买更多不同规格的标准版实例，标准实例 24 小时运行随时待命。同时还提供了负载均衡、组管理、预备环境、平滑部署等管理功能，充分保障服务的可用性：

- 可以按需定制实例的数量和规格，如果你创建了两个或更多的实例便可实现 **负载均衡**，通过添加实例数量简单地提升处理能力；多个实例会自动分配到不同的物理服务器上，可以在单个实例出现故障时进行自动的的故障转移（failover），例如当一个实例崩溃重启，或所在服务器出现意外的硬件故障，请求会自动交由其他实例处理，保障服务的 **高可用性**。
- 可以 [创建多个分组](#组管理)，部署不同代码（甚至不同的语言），绑定不同的域名。
- 在进行部署或管理操作时，会先启动新版本容器，待负载均衡切换、正在进行的请求处理结束再关闭旧容器，服务零中断。
- 每个分组均提供一个免费的预备环境实例，在正式上线之前，你可以先将代码发布到预备环境，使用线上的环境和数据进行测试。

如不希望继续付费，可删除应用下所有生产环境中的标准实例，会自动切换回体验实例，不再收取费用。

## 休眠策略

<div class="callout callout-info">生产环境的标准实例不会休眠。</div>
